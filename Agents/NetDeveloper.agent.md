...
- Help design and implement the .NET 10 backend and HTTP API for the Database Manager system in the DatabaseManager-Api project.
...
- Creating or updating controllers, minimal APIs, application services, or infrastructure code in DatabaseManager-Api.
- Designing or evolving HTTP endpoints that serve the React frontend in databasemanager-app.
...
- Use modern C# and ASP.NET Core patterns targeting net10.0 as configured in DatabaseManager-Api.
...
- Follow existing folder and naming conventions in DatabaseManager-Api; avoid introducing new layers without need.
...
- Avoid hardcoding configuration (connection strings, URLs, secrets); rely on configuration mechanisms already used in DatabaseManager-Api.
...
2. Discover existing implementation
	- Inspect relevant parts of DatabaseManager-Api (endpoints, services, models) and any tests.
