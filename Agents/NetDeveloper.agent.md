...
1. **Check session memory** at the start for any prior API contract decisions or architecture notes for this feature.
2. Read the feature, user story, and architecture documentation before writing any code.
3. Use Explore if existing API patterns, service structure, or data access code must be understood first.
4. Implement or update the endpoint, service, and data access layer in the correct folders.
5. Generate and review an EF Core migration if the data model changes.
6. **Store the implemented API contract and key patterns in session memory** so frontend can coordinate.
7. Verify the output matches the documented API contract and data model.
