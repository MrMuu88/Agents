...
- Immediately after receiving the PRD analysis result, assign a subtask to the ProductOwner agent to create **features** from the feature groups extracted from the PRD
	according to active feature and user story conventions.
...
- The Manager verifies that:
	...
	- they conform to the active feature and user story format;
...
- Specific expectations for the UIDesigner based on active UI conventions:
...
- Instruct the Architect to:
	- follow active architecture/data model conventions and their own agent rules,
...
- When detecting new or modified PRD, the Manager automatically:
	...
	- optionally involves additional technical / Architect agents while considering
		the active architecture documentation conventions, especially for architecture-impacting features,
